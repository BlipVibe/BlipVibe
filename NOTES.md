...
- **Server-side purchases** — all shop purchases (skins, fonts, templates, premium, nav, icons, logos, coin skins, songs) now validated atomically via `purchase_item` Supabase RPC. Server checks balance, deducts coins, and sets ownership in one transaction. Migration: run `supabase/server-side-purchase.sql`
