...
- Removed `blipvibe_cache_{userId}` localStorage — Supabase is now the sole source of truth for all skin/customization data. No more stale cache causing skin bleed, background copying, or settings mismatch between profiles. `_buildSkinData()` simplified to always read from current state (sync is already blocked during profile/group view).
...
- **Skin state overhaul** — `state.activePremiumSkin` is NEVER mutated when viewing other profiles or groups. `updatePremiumBg(forceShow)` accepts a parameter instead of reading from state. Profile view and group view both restore from Supabase on exit. `_pvRealSkin` provides belt-and-suspenders backup for `_buildSkinData`. Group view restore now uses Supabase instead of local snapshot.
...
## Inline Video Thumbnails Reliably Render (v0.6.6 — 2026-04-25)
- **Symptom reported:** Users uploading a video saw the post appear in the feed but no video — just an empty / black box where the thumbnail should be.
- **Root cause:** `buildMediaGrid()` (`js/app.js` ~line 6170) rendered inline videos as `<video src="...mp4#t=0.5" preload="metadata" muted playsinline>`. The `#t=0.5` Media Fragment is supposed to make the browser show the frame at 0.5s as a poster — but with `preload="metadata"` only the first few KB of metadata is fetched, which is not enough to decode any frame. Many browsers (notably iOS Safari WebView) never paint a frame and leave the video element black until the user explicitly plays it. To users, that looks identical to "video didn't upload."
- **Fix 1 (JS):** Added `onloadedmetadata="this.currentTime=Math.min(0.5,(this.duration||1)-0.01);"` to the inline `<video>` HTML in `buildMediaGrid()`. Once metadata is loaded, this forces the video element to seek to 0.5s (or just shy of the end for very short clips), which forces the browser to fetch the bytes needed to decode and paint that frame. The thumbnail now reliably renders the frame at 0.5s as a poster.
- **Fix 2 (CSS):** `.pm-count-1 .pm-thumb` had only `max-height:500px` and no min-height; combined with `<video height:auto>` this could collapse to ~0px while metadata was loading. Added `min-height:240px` to both `.pm-count-1 .pm-thumb` and `.pm-count-1 .pm-thumb video` so single-video posts always reserve visible space.
- **Files changed:** `js/app.js` (buildMediaGrid), `css/style.css` (.pm-count-1 thumb min-heights)
- **Note for future debug:** the `<video>` tags created by `buildMediaGrid` do *not* have a `controls` attribute, so the existing scroll-pause IntersectionObserver (which selects `video[controls]`) does not observe them — that's intentional, the inline videos are visual-only thumbnails that open the lightbox on tap.

## Sticky Error Banner for Upload Failures (v0.6.6 — 2026-04-26)
- **Why:** The catch blocks around `sbUploadPostVideo` / `sbUploadPostImage` in the main composer (line ~7222) and the group composer (line ~5270) used `showToast(...)` for failures. The toast auto-dismisses in 2.5s, easy to miss — especially on mobile where the publish UI is busy. Users would see "post created with no video" and not realize the upload had silently errored.
- **Added:** `showStickyError(msg)` helper next to `showToast`. Renders a fixed red banner at the top of the screen (respects `env(safe-area-inset-top)` for iPhone notch), z-index 99999, with a × close. Tap to dismiss. Replacing an existing sticky error replaces the DOM node instead of stacking.
